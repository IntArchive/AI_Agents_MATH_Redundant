...
--folder_to_save_json "./data/numberofassumption_and_Groundtruth_redundant_assumption_number" \
--file_json_without_extension "Dataset_numberofassumption_and_Groundtruth_redundant_assumption_number"
"""
...
def define_the_number_of_assumption(row, input_original_problem_col: str = 'Original_Problem', output_number_of_assumption_col: str = 'Number_of_Assumption'):
    """
    Define the number of assumption from the original problem.
    input_original_problem_col: str = 'Original_Problem'
    output_number_of_assumption_col: str = 'Number_of_Assumption'
    """
    assumption = re.search(PATTERN_DETECT_ASSUMPTION, row[input_original_problem_col])
    if assumption:
        assumptions = assumption.group(0).replace("Assumption:", "").replace("Problem:", "").replace("problem:", "").replace("assumption:", "").strip()
    ...
    row[output_number_of_assumption_col] = len(list_of_assumptions)
    return row

def define_the_number_of_redundant_assumption(row, input_col: str = 'Number_of_Assumption', output_number_of_redundant_assumption_col: str = 'Groundtruth_redundant_assumption_number'):
    """
    Define the number of redundant assumption from the original problem.
    input_redundant_assumption_col: str = 'Redundant_assumption'
    output_number_of_redundant_assumption_col: str = 'Number_of_Redundant_Assumption'
    """
    number_of_redundant_assumption = row[input_col] + 1
    row[output_number_of_redundant_assumption_col] = number_of_redundant_assumption
    return row
...
def main():
    ...
    data = data.apply(lambda row: define_the_number_of_assumption(row, input_original_problem_col="Original_Problem", output_number_of_assumption_col="Number_of_Assumption"), axis=1)
    data = data.apply(lambda row: define_the_number_of_redundant_assumption(row, input_col="Number_of_Assumption", output_number_of_redundant_assumption_col="Groundtruth_redundant_assumption_number"), axis=1)
    dataframe = []
    ...
    for index, row in data.iterrows():
        json_data = {
            ...
            "Groundtruth_redundant_assumption": row["Redundant_assumption"],
        ...
        dataframe.append(json_data)
    dataframe = pd.DataFrame(dataframe)
    dataframe.to_excel(args.filechuakiemtra_originalproblem.replace(".xlsx", "_with_numerical_assumption.xlsx"), index=False)
