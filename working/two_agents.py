...
def main():    
    ...
    If it has a redundant assumption, create a new problem that we can deduce the redundant assumption from the other assumptions.
    ...
    ###END_OF_FORMAT_PART1###
    To prove an assumption is redundant, we usually consider it as important result which can be deduced from the other given assumptions. Based on that logic, we will construct a new problem which have assumptions from the original problem exclude the redundant assumption we suppose and the redundant assumption as an important result we need to prove it right.
    ...
    reviewer, reviewer_system = build_agent(
    ...
    system = MultiAgentSystem(
        roles=[
            role("judge", judge, judge_system),
            role("final reviewer", reviewer, reviewer_system),
        ],
